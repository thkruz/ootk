/**
 * @file   Tests from Sgp4Js.js to ensure compatibility
 * @since  0.2.0
 */
...
describe.skip('Julian date / time', () => {
  ...
  describe('jday & invjday', () => {
    it('gives the same result with different arguments describing the same time', () => {
      expect(Sgp4Js.jday(now)).toEqual(
        Sgp4Js.jday(
          now.getUTCFullYear(),
          now.getUTCMonth() + 1,
          now.getUTCDate(),
    ...
    it('invjday gives the same result as date and array', () => {
      const jd = Sgp4Js.jday(now);
      const date = Sgp4Js.invjday(jd);
      const dateArray = Sgp4Js.invjday(jd, true);
    ...
    it('date to jday and inverse conversion', () => {
      ...
      expect(Sgp4Js.invjday(jd).getTime() / 1000).toBeCloseTo(expected, -1);
    });
  });

  it('gstime gives the same result with different arguments describing the same time', () => {
    expect(Sgp4Js.gstime(now)).toEqual(
      Sgp4Js.gstime(
        now.getUTCFullYear(),
        now.getUTCMonth() + 1,
        now.getUTCDate(),
  ...
  it('propagation gives the same result with different arguments describing the same time', () => {
    ...
    const satrec = Sgp4Js.twoline2rv(tleLine1, tleLine2);

    const propagationByDate = Sgp4Js.propagate(satrec, date);
    const propagationByDateItems = Sgp4Js.propagate(
      satrec,
      date.getUTCFullYear(),
      date.getUTCMonth() + 1,
      date.getUTCDate(),
