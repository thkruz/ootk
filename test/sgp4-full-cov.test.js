...
import { Sgp4Js } from '../src/sgp4.js';
// import { Sgp4JsUtils } from '../src/sgp4-utils.js';
...
describe('Verification TLE Data in Appendix D of Revisiting Spacetrack Report #3: Rev 1', () => {
  sgp4Data.forEach((sgp4DataItem) => {
    ...
    const satrec = Sgp4Js.twoline2rv(sgp4DataItem.tleLine1, sgp4DataItem.tleLine2);
    ...
    if (sgp4DataItem.error) {
      ...
      test(`if ${sgp4DataItem.description} passes`, () => {
        sgp4DataItem.results.forEach((expected) => {
          const sgp4Result = Sgp4Js.sgp4(satrec, expected.time, 'a');
...
describe('Test vector equations in SGP4', () => {
  test('if mag computes a magnitude', () => {
    expect(Sgp4Js.mag([10, 10, 10])).toEqual(17.320508075688775);
  });
  test('if cross computes a cross product', () => {
    expect(Sgp4Js.cross([10, 10, 10], [-10, -10, -10])).toEqual([0, 0, 0]);
  });
  test('if dot computes a dot product', () => {
    expect(Sgp4Js.dot([10, 10, 10], [-10, -10, -10])).toEqual(-300);
  });
  test('if angle computes an angle in radians', () => {
    expect(Sgp4Js.angle([10, 10, 10], [-10, -10, -10])).toEqual(3.1415926325163688);
  });
  test('if angle returns a huge number when it is the same value', () => {
    ...
    expect(Sgp4Js.angle([0, 0, 10], [0, 0, 0])).toEqual(999999.1);
  });
  test('if asinh computes...', () => {
    expect(Sgp4Js.asinh(Math.PI)).toEqual(1.8622957433108482);
  });
  test('if newtonnu works for circular orbits', () => {
    expect(Sgp4Js.newtonnu(0, 0)).toEqual({ e0: 0, m: 0 });
  });
  test('if newtonnu works for elliptical orbits', () => {
    expect(Sgp4Js.newtonnu(0.5, 0)).toEqual({ e0: 0, m: 0 });
  });
  test('if newtonnu works for hyperbolic orbits', () => {
    expect(Sgp4Js.newtonnu(1, 0)).toEqual({ e0: 0, m: 0 });
  });
  test('if newtonnu works for parabolic orbits', () => {
    expect(Sgp4Js.newtonnu(1.5, 0)).toEqual({ e0: 0, m: 0 });
  });
  test('if newtonnu works for negative orbits', () => {
    expect(Sgp4Js.newtonnu(0, -1)).toEqual({ e0: -1, m: 5.283185307179586 });
  });
  ...
  test('if invjday calculates date', () => {
    expect(Sgp4Js.invjday(2450000, 0)).toEqual({
      day: 9,
      hr: 12,
      minute: 0,
      mon: 10,
    ...
    expect(Sgp4Js.jday(1995, 10, 9, 12, 0, 0)).toEqual({
      jd: 2449999.5,
      jdFrac: 0.5,
    });
    ...
    let jday2 = Sgp4Js.jday(1995, 10, 9, 12, 0, 0);
...
//     expect(Sgp4JsUtils.degreesLat(0.5)).toEqual(28.64788975654116);
//   });
//   test('If degreesLong is accurate', () => {
//     expect(Sgp4JsUtils.degreesLong(0.5)).toEqual(28.64788975654116);
//   });
//   test('If radiansLat is accurate', () => {
//     expect(Sgp4JsUtils.radiansLat(45)).toEqual(0.7853981633974483);
//   });
//   test('If radiansLong is accurate', () => {
//     expect(Sgp4JsUtils.radiansLong(45)).toEqual(0.7853981633974483);
//   });

//   test(`if sgp4-utils checks for errors`, () => {
//     expect(() => Sgp4JsUtils.degreesLat(-100)).toThrowError(
//       new RangeError('Latitude radians must be in range [-PI/2; PI/2].')
//     );
//     expect(() => Sgp4JsUtils.degreesLat(100)).toThrowError(
//       new RangeError('Latitude radians must be in range [-PI/2; PI/2].')
//     );
//     expect(() => Sgp4JsUtils.degreesLong(-100)).toThrowError(
...
//     expect(() => Sgp4JsUtils.radiansLat(-100)).toThrowError(
//       new RangeError('Latitude degrees must be in range [-90; 90].')
//     );
//     expect(() => Sgp4JsUtils.radiansLat(100)).toThrowError(
//       new RangeError('Latitude degrees must be in range [-90; 90].')
//     );
//     expect(() => Sgp4JsUtils.radiansLong(-300)).toThrowError(
...
describe('Ensure bstar and ndot account for leading zeros', () => {
  test('if bstar reads in leading zero without exponent', () => {
    ...
    const line2 = '2 00005  34.2682 348.7242 1859667 331.7664  19.3264 10.82419157413667';
    const satrec = Sgp4Js.twoline2rv(line1, line2, 'wgs72', 'i');
