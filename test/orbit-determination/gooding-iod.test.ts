...
describe('GoodingIOD', () => {
  ...
  describe('Orbital regime tests', () => {
    ...
    it('should handle real GEO observations', () => {
      // Create a known NAVSTAR-like MEO orbit
      const originalTle = new Tle(
        '1 35873U 09050A   25326.33914853 -.00000235  00000+0  00000+0 0 99993',
        '2 35873   0.0126 286.1820 0002783 320.7841 224.0129  1.00270158 59276',
      );
      ...
      const t1 = EpochUTC.fromDateTimeString('2025-11-22T17:25:30.000Z');
      const t2 = EpochUTC.fromDateTimeString('2025-11-22T17:26:30.000Z');
      const t3 = EpochUTC.fromDateTimeString('2025-11-22T17:27:30.000Z');
      // Generate observations from known orbit
      const raDec1a = RadecTopocentric.fromDegrees(
        t1, 256.7721 as Degrees, -6.4951 as Degrees,
      );
      const raDec2a = RadecTopocentric.fromDegrees(
        t2, 256.9885 as Degrees, -6.4950 as Degrees,
      );
      const raDec3a = RadecTopocentric.fromDegrees(
        t3, 257.2387 as Degrees, -6.4948 as Degrees,
      );

      const obs1a = new ObservationOptical(sensor.toJ2000(t1.toDateTime()), raDec1a);
      const obs2a = new ObservationOptical(sensor.toJ2000(t2.toDateTime()), raDec2a);
      const obs3a = new ObservationOptical(sensor.toJ2000(t3.toDateTime()), raDec3a);

      // Generate observations from known orbit
      const raDec1b = RadecTopocentric.fromStateVector(propagator.propagate(t1), sensor.toJ2000(t1.toDateTime()));
      const raDec2b = RadecTopocentric.fromStateVector(propagator.propagate(t2), sensor.toJ2000(t2.toDateTime()));
      const raDec3b = RadecTopocentric.fromStateVector(propagator.propagate(t3), sensor.toJ2000(t3.toDateTime()));

      const los1a = raDec1a.lineOfSight();
      const los1b = raDec1b.lineOfSight();

      console.log('Angle between:', los1a.angle(los1b) * RAD2DEG, 'degrees');

      const obs1b = new ObservationOptical(sensor.toJ2000(t1.toDateTime()), raDec1b);
      const obs2b = new ObservationOptical(sensor.toJ2000(t2.toDateTime()), raDec2b);
      const obs3b = new ObservationOptical(sensor.toJ2000(t3.toDateTime()), raDec3b);

      const iod = new ModifiedGoodingIOD();

      const orbita = iod.solve([obs1a, obs2a, obs3a], 376300 as Kilometers, 376300 as Kilometers);
      const orbitb = iod.solve([obs1b, obs2b, obs3b], 376300 as Kilometers, 376300 as Kilometers);

      const elements = orbita.toClassicalElements();
      const elementsb = orbitb.toClassicalElements();

      // Should be close to the original GEO orbit
      expect(Math.abs(elements.semimajorAxis - originalTle.semimajorAxis)).toBeLessThanOrEqual(50);
      expect(Math.abs(elements.eccentricity - originalTle.eccentricity)).toBeLessThanOrEqual(0.01);
      expect(Math.abs(elements.inclinationDegrees - originalTle.inclinationDegrees)).toBeLessThanOrEqual(0.5);
    });
  });
});
